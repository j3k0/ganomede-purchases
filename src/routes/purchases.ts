...
export const addRoute = (purchasesStore: PurchasesStore, customersClient: V2.CustomersClient) => (prefix: string, server: Server, authdbClient: AuthdbClient | null) => {

  server.get(`${prefix}/auth/:token/subscription`,
    requireAuth({ authdbClient, secret: '' }),
    getLastSubscription(purchasesStore, customersClient))
};
